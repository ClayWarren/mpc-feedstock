@echo on
cl /nologo /W4 test-native.c /I"%LIBRARY_INC%" /Fe:consumer-msvc.exe /link /LIBPATH:"%LIBRARY_LIB%" mpc.lib mpfr.lib gmp.lib
if errorlevel 1 exit /b 1
consumer-msvc.exe
if errorlevel 1 exit /b 1
set "EXPECTED_MACHINE=8664 machine (x64)"
set "NATIVE_DLL=libmpc-3.dll"
if "%target_platform%"=="win-arm64" (
    set "EXPECTED_MACHINE=AA64 machine (ARM64)"
    set "NATIVE_DLL=mpc-3.dll"
)
...
if errorlevel 1 exit /b 1
if "%target_platform%"=="win-arm64" (
    call %CC% %CFLAGS% -I"%LIBRARY_INC%" test-native.c -L"%LIBRARY_LIB%" -lmpc -lmpfr -lgmp %LDFLAGS% -o consumer-clang.exe
    if errorlevel 1 exit /b 1
    consumer-clang.exe
    if errorlevel 1 exit /b 1
