...
local = Path("C:/mpc-local")
conda = [sys.executable, "C:/mpc-tools/Scripts/conda-script.py"]
...
run(conda + ["build", "recipe", "-m", f".ci_support/{subdir.replace('-', '_')}_.yaml",
             "--variants", json.dumps({"build_platform": subdir}),
             "--croot", "C:/mpc-build", "--output-folder", str(local), "--no-anaconda-upload",
             "--override-channels", "-c", "file:///C:/mpc-local", "-c", "conda-forge"])
print(f"PASS: mpc build and installed consumer tests on {subdir}", flush=True)
